...
class ProductHuntCollector(BaseCollector):
    ...
    async def _graphql_request(
        ...
        logger.debug(f"[HTTP Request] POST {self.BASE_URL} (GraphQL)")
        ...
        try:
            async with self.session.post(
                ...
                logger.info(f"[HTTP] {status_emoji} POST {self.BASE_URL} -> {response.status} ({elapsed:.2f}s)")
                
                if response.status == 401:
                    logger.error("[HTTP Auth Error] Product Hunt authentication failed")
                    return None
                ...
                if data.get('errors'):
                    logger.error(f"[GraphQL Error] Product Hunt returned errors: {data['errors']}")
                
                return data.get('data')
                
        except Exception as e:
            elapsed = time.time() - start_time
            logger.error(f"[HTTP Error] POST {self.BASE_URL} failed after {elapsed:.2f}s: {e}")
            return None
