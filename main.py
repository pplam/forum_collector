...
from web_view import run_web_view
...
class ForumCollectorApp:
    ...
    def __init__(self, config: Settings):
        ...
        self.storage = StorageManager(
            storage_type=config.storage_type, storage_path=config.storage_path
        )
        self._collectors: Dict[str, BaseCollector] = {}

    def _get_collector(
        self, name: str, collector_config: CollectorConfig
        ...
        if collector_config.source == ForumSource.HACKER_NEWS:
            ...
            return GitHubDiscussionsCollector(
                collection_config, token=collector_config.access_token
            )
            ...
            return ProductHuntCollector(
                collection_config, api_key=collector_config.api_key
            )
        ...
        elif collector_config.source == ForumSource.INDIE_HACKERS:
            return IndieHackersCollector(collection_config)
        else:
            logger.warning(f"Unsupported source: {collector_config.source}")
            return None

    async def collect_from_source(
        self, source_name: str, limit: Optional[int] = None
    ) -> CollectionResult:
    ...
    while True:
        ...
        if choice == "0":
            ...
            for hp in hot_posts:
                ...
                print(
                    f"   Score: {hp.post.score:.1f} | Trending: {hp.trending_score:.2f} | "
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Forum Collector - Collect hot posts from various forums"
    )

    parser.add_argument("-c", "--config", help="Path to configuration file")
    parser.add_argument("-s", "--source", help="Collect from a specific source only")
    parser.add_argument(
        "-l",
        "--limit",
        type=int,
    ...
    parser.add_argument("--search", help="Search for posts matching a query")
    parser.add_argument("--stats", action="store_true", help="Show storage statistics")
    parser.add_argument(
        "--cleanup", type=int, help="Delete posts older than specified days"
    )
    parser.add_argument(
    ...
    parser.add_argument("--generate-config", help="Generate default configuration file")
    parser.add_argument(
        "--web", action="store_true", help="Start web server to explore posts"
    )
    parser.add_argument(
        "--web-host", default="127.0.0.1", help="Web server host (default: 127.0.0.1)"
    )
    parser.add_argument(
        "--web-port", type=int, default=5000, help="Web server port (default: 5000)"
    )
    ...
    # Set up logging
    setup_logging(
        level=args.log_level, log_file="forum_collector.log", log_dir="./logs"
    )

    # Load configuration
    config = load_config(args.config)

    # Set up proxy from config if enabled
    ...
    try:
        if args.interactive:
            ...
            run_web_view(app.storage, host=args.web_host, port=args.web_port)
